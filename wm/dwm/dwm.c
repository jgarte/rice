...
{
	ltext = &rtext;
	if (!gettextprop(root, XA_WM_NAME, rtext, sizeof(rtext))) {
		strcpy(rtext, "dwm-"VERSION);
	} else {
		for(short c = 0; c < sizeof(rtext); c++) {
			if (rtext[c] == barsep) {
				rtext[c] = '\0';
				ltext = &rtext[c + 1];
				break;
...
tile(Monitor *m)
{
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + gappx, m->wy + my, mw - (2*c->bw) - gappx, h - (2*c->bw) - gappx, 0);
			my += HEIGHT(c) + gappx;
		} else {
			r = n - i;
			h = (m->wh - ty - gappx * (r - 1)) / r;
			resize(c, m->wx + mw + gappx, m->wy + ty, m->ww - mw - (2*c->bw) - (2*gappx), h - (2*c->bw) - gappx, 0);
			ty += HEIGHT(c) + gappx;
		}
}
...
{
	unsigned int i, n, w, g = 0, mh, mw, mx, mxo, my, myo, tx;
	Client *c;
